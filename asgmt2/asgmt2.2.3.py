...
def plt_asgmt222(b, alpha, omega, eps=1e-10, new_ver=False):
    output_jac = calc_iterative_method(A, b, alpha, EPS=eps, new_ver=new_ver)
    output_gau = calc_iterative_method(A, b, alpha, method='gauss_seidel', EPS=eps, new_ver=new_ver)
    output_sor = calc_iterative_method(A, b, alpha, method='sor', omega=omega, EPS=eps, new_ver=new_ver)  # 1: 1.852, 2: , 3: 1
...
def main():
    plt_asgmt222(b1, alpha1, omega=1.852, new_ver=True)
    plt_asgmt222(b2, alpha2, omega=1.9, new_ver=True)
    plt_asgmt222(b3, alpha3, omega=1.8, new_ver=True)
